...
public class ResourceByCategoryAdapter extends RecyclerView.Adapter<ResourceByCategoryAdapter.ViewHolder> implements View.OnClickListener, View.OnLongClickListener {
    private List<TeacherResChapterBean.DataBean> DataList;
     ...
     * @param DataList
     */
    public ResourceByCategoryAdapter(Context mContext, List<TeacherResChapterBean.DataBean> DataList) {
        this.DataList = DataList;
        this.mContext = mContext;

    }

    public void refreshData(List<TeacherResChapterBean.DataBean> DataList) {
        this.DataList = DataList;
        if (!isclick)
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_resourcebycategory, parent, false);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        TeacherResChapterBean.DataBean bean = DataList.get(position);

        holder.Name.setText(bean.getName());
        if (TextUtils.isEmpty(bean.getDuration())||Integer.parseInt(bean.getDuration())==0) {
            holder.ShijianLine.setVisibility(View.GONE);
        } else {
            holder.ShijianLine.setVisibility(View.VISIBLE);
            DataUtils getData = new DataUtils();
            holder.Shijian.setText(getData.toTimeStr(Integer.parseInt(bean.getDuration())));
        }

        if (TextUtils.isEmpty(bean.getFileSize())) {
            holder.DaxiaoLine.setVisibility(View.GONE);
        } else {
            holder.DaxiaoLine.setVisibility(View.VISIBLE);
            DateUtils dateUtils = new DateUtils();

            holder.Daxiao.setText(dateUtils.formetFileSize(Double.parseDouble(bean.getFileSize())));
        }
