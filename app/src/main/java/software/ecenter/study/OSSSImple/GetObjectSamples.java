...
import software.ecenter.study.OssConfig.Callback;
...
public class GetObjectSamples {
    ...
    //异步下载文件的方法
    public void asyncGetObjectSample(GetObjectRequest objectRequest,final OSSCompletedCallback<GetObjectRequest, GetObjectResult> callback) {

        GetObjectRequest get = objectRequest;
        get.setProgressListener(new OSSProgressCallback<GetObjectRequest>() {
            @Override
            public void onProgress(GetObjectRequest request, long currentSize, long totalSize) {
                Log.e("GetObjectSamples","getobj_progress: " + currentSize + "  total_size: " + totalSize);
            }
        });
        oss.asyncGetObject(get,callback);
        /*OSSAsyncTask p = oss.asyncGetObject(get, new OSSCompletedCallback<GetObjectRequest, GetObjectResult>() {
            @Override
            public void onSuccess(GetObjectRequest request, GetObjectResult result) {
                ...
                try {
                    while ((len = inputStream.read(buffer)) != -1) {
                        ...
                        OSSLog.logD("asyncGetObjectSample-------read length: " + len);
                    }
                    OSSLog.logD("asyncGetObjectSample-----download success.");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
