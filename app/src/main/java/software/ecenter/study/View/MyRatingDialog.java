...
public class MyRatingDialog extends Dialog {
	...
	public MyRatingDialog(Context context, int theme) {
		super(context, theme);
	}

	public MyRatingDialog(Context context) {
		super(context);
	}
	...
	public static class Builder {
		...
		public Builder setStar(float star) {
			this.star = star;
			return this;
		}
		...
		public MyRatingDialog create() {
			LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
			// instantiate the dialog with the custom Theme
			final MyRatingDialog dialog = new MyRatingDialog(context, R.style.MyDialog);
			View layout = inflater.inflate(R.layout.layout_rating_dialog, null);
				...
				if (positiveButtonClickListener != null) {
					((TextView) layout.findViewById(R.id.dialog_ok)).setOnClickListener(new View.OnClickListener() {
						public void onClick(View v) {
							positiveButtonClickListener.onClick(dialog, DialogInterface.BUTTON_POSITIVE,getStar());
						}
					});
				}
			} else {
			...
			if (negativeButtonText != null) {
				...
				if (negativeButtonClickListener != null) {
					((TextView) layout.findViewById(R.id.dialog_quite)).setOnClickListener(new View.OnClickListener() {
						public void onClick(View v) {
							negativeButtonClickListener.onClick(dialog, DialogInterface.BUTTON_NEGATIVE,getStar());
						}
					});
				}
			} else {
			...
			mRatinbar = layout.findViewById(R.id.ratingView);
			// set the content message
			if (star != -1) {
				mRatinbar.setRating(star);
	...
	public interface OnClickListener {
		void onClick(DialogInterface dialog, int which,float star);
	}
