...
public class PinyinKenchenListActivity extends BaseActivity{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        initListener();
        adapter = new PykechenAdapter(mContext,new OnItemListener(){
            @Override
...
//                    startActivity(new Intent(mContext, PinyinDetailActivity.class));
    ...
    @OnClick({R.id.iv_back, R.id.tv_xuan})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.tv_xuan://炫耀
                startActivity(new Intent(mContext, SharePinActivity.class).putExtra("score",adapter.getScore()).putExtra("month",month));
                break;
        }
    }
    @Override
    ...
    private void initListener() {
        PlayHelperUtil.newInrence().listener(new PlayerCallback() {
            ...
            public void onError(Object dataSource, AudioPlayerManager manager) {
                adapter.setPlay(playPoc,PlayHelperUtil.newInrence().playStatus());
                ToastUtils.showToastSHORT(mContext,"播放错误");
            }
